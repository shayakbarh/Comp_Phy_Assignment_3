...
void plotTwoCurves(double *x1, double *y1, double *x2, double *y2, int size) {
    ...
    if (gnuplotPipe) {
        ...
        fprintf(gnuplotPipe, "plot '-' with lines title 'Numerical FT', '-' with lines title 'Analytical FT'\n");
...
// Function to calculate analytic FT
...
int main() {
    ...
    FILE *file = fopen("fftw_FT_sinc.txt", "w");
