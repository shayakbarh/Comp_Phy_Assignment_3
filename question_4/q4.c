...
// Function to plot two curves using Gnuplot
void plotTwoCurves(double *x1, double *y1, double *x2, double *y2, int size) {
    FILE *gnuplotPipe = popen("gnuplot -persistent", "w");

    if (gnuplotPipe) {
        // Send commands to Gnuplot to plot two curves
        fprintf(gnuplotPipe, "plot '-' with lines title 'Numerical Fourier Transform', '-' with lines title 'Analytical Fourier Transform'\n");
...
// Function for analytical fourier transform
double* analytic_FT(double *x, int size) {
...
int main() {
    ...
    double *f_analytic = analytic_FT(k_values,NUM_POINTS);
    ...
    FILE *file = fopen("fftw_FT_gauss.txt", "w");
    ...
    //plot the two curves
    plotTwoCurves(k_values,FT,k_values,f_analytic,NUM_POINTS);
